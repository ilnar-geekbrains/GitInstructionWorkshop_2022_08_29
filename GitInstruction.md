...
Репозиторий - место, где хранятся и поддерживаются какие-либо данные, с возможностью контроля версий.

Для начала работы с репозиторием надо открыть в VSCode и в нём открыть папку с помощью кнопки "открыть папку", либо Файл -> Открыть папку. (Или CTRL+K CTRL+O)

После чего надо инициализировать папку. Для этого надо открыть терминал. Терминал -> Создать терминал (CTRL+SHIFT+')

После чего в терминале пишем git init

И всё. Папка инициализитровалась.
