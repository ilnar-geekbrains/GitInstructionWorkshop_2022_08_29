...
До этого мы работали только с локальным репозиторием, теперь освоим удаленный. Удаленный репозиторий нужен чаще всего для совместной работы, которую можно будет кооперировать на отдельном сервере, а не как было на одном устройстве до этого. В наличие есть различные сервисы, но самый часто используемым является __GitHub__.
